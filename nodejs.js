...
client.on("ready", () => {
	...
	let status = " Aktif!";
	if (db.get("owosystemstatus") == "0") { //made by DioSShop, software version 8.4.1
		status = "Aktif Değil";
	} 
	console.log("//made by DioSShop, software version 8.4.1 OwoHunt Bot Çalışıyor | Anlık durum: "+status);
	...
	}////made by DioSShop, software version 8.4.1
});

setInterval(function() {
	if (Number(db.get("securitytime")) > 10 || Number(db.get("securitytime")) < 1) {
		db.set("securitytime", "3");
		client.channels.get(db.get("channelid")).send("Güvenlik ayarları hatalı girildiğinden tekrar yapılandırıldı."); 
	}
	if (db.get("captchasecurity") == "1") {
		if (db.get("owosecurity") == "0") {
			...
			if (db.get("owosystemstatus") == "0") {
				...
				client.channels.get(db.get("channelid")).send("Bot tekrardan aktif edilmiştir."); 
				return;////made by DioSShop, software version 8.4.1
			}
		}
	}
	...
	if (db.get("owosystemstatus") == "1") {
		if (db.get("autosell") == "1") {
			client.channels.get(db.get("channelid")).send("Otomatik satım işlemi yapılıyor.").then(msg => {
				...
				}, 500);////made by DioSShop, software version 8.4.1
			});
			return;
		}
...
setInterval(function(){
	if (db.get("owosystemstatus") == "1") {
		if (db.get("systems.wkiss") == "1") {
			db.add("kiss", 1);
			client.channels.get(db.get("channelid")).send("owo kiss <@408785106942164992>");////made by DioSShop, software version 8.4.1
		}
	}
}, kiss);
...
setInterval(function(){
	if (db.get("owosystemstatus") == "1") {
		if (db.get("systems.wcf") == "1") {////made by DioSShop, software version 8.4.1
			db.add("cf", 1);
			client.channels.get(db.get("channelid")).send("owo cf 100");
		}
	}
}, cf);
...
client.on("message", message => {
	...
	let hugis = db.get("hug")////made by discord.gg/novapattis, software version 8.4.1
	let cfis = db.get("cf")



	let huntdurum = "Aktif!";
	if (db.get("systems.whunt") == "0") { 
	...
	}////made by discord.gg/novapattis, software version 8.4.1
		...
		if (message.content.indexOf(client.user.username) != "-1") {
			...
			if (db.get("owomessagetype") == "çek") {
				if (Number(cash) < 3000) {
					...
					db.set("owomessagetype", "0"); //made by discord.gg/novapattisd, software version 8.4.1
					db.set("owomessagestatus", "0");
					return;
				}
				message.channel.send("owo give <@"+db.get("ownerid")+"> "+(Number(cash)-1000)).then(msg => {
					msg.delete();////made by discord.gg/novapattis, software version 8.4.1
				});
			...
			message.channel.send("Toplamda "+cash+" owo parası bulunuyor.");////made by discord.gg/novapattis, software version 8.4.1
			db.set("owomessagestatus", "0");
			message.delete();
		}
...
client.on("message", message => {
	...
	if (message.channel.type == "dm") {
		if (message.content.indexOf("Are you a real human? Please use the link below so I can check!") != "-1") {
			...
			return;////made by discord.gg/novapattis, software version 8.4.1
		}
		...
		if (message.content.indexOf("Wrong verification code!") != "-1") {
			...
			db.set("owosystemstatus", "0");
			db.set("owosecurity", "1");////made by discord.gg/novapattis, software version 8.4.1////made by discord.gg/novapattis, software version 8.4.1
			return;
		}
...
client.on("message", message => {
	...
	if (message.content.split(" ")[0] == "!kanalayarla") {
		...
		if (channelget) {
			...
			message.channel.send("Kanal başarıyla ayarlandı. Artık <#"+channelget.id+"> kanalında çalışacağım.İ Work İn Here.");
		} else {
		...
		db.set("owosystemstatus", "1");
	}////made by discord.gg/novapattis, software version 8.4.1////made by discord.gg/novapattis, software version 8.4.1
	...
	if (message.content.split(" ")[0] == "!cash") {////made by discord.gg/novapattis, software version 8.4.1
	...
	if (message.content.split(" ")[0] == "!komut-ayarla") {
		if (message.content.split(" ")[1] == "whunt") {
			...
			return; //made by discord.gg/novapattis, software version 8.4.1////made by discord.gg/novapattis, software version 8.4.1
		}
		...
		if (message.content.split(" ")[1] == "whug") {////made by discord.gg/novapattis, software version 8.4.1
			let status = "3";
			if (message.content.split(" ")[2] == "aç") status = "1";
			if (message.content.split(" ")[2] == "kapat") status = "0";
			if (status == "3") {
		...
		if (message.content.split(" ")[1] == "wcf") {
			let status = "3";
			if (message.content.split(" ")[2] == "aç") status = "1";
			if (message.content.split(" ")[2] == "kapat") status = "0";////made by discord.gg/novapattis, software version 8.4.1
			if (status == "3") {
				message.channel.send("⛔ Lütfen geçerli argüman girin. `aç` veya `kapat` yazabilirsiniz.");
				return;
			}
	...
	if (message.content.split(" ")[0] == "!ayarlar") {
		...
		let wbattle = "✅ Aktif!";
		if (db.get("systems.wbattle") == "0") wbattle = "⛔ Aktif Değil!";////made by discord.gg/novapattis, software version 8.4.1////made by discord.gg/novapattis, software version 8.4.1
		let whug = "✅ Aktif!";
	...
	if (message.content.split(" ")[0] == "!botuyenile") {////made by discord.gg/novapattis, software version 8.4.1
		process.exit(1)
	}
	...
	if (message.content.split(" ")[0] == "!durdur") {
		db.set("owosystemstatus", "0");
		message.channel.send("✅ Bot başarıyla durdurulmuştur. !aktifet yazarak tekrardan aktif edebilirsiniz.");////made by discord.gg/novapattis, software version 8.4.1
		return;
	}
